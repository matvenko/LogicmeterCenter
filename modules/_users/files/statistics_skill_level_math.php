...
global $out;
//*** search fields
$replace_fields['statistic_page'] = "statistics_skill_level_math";
...
//***** skills *****
$grade_tmpl = $templates->split_template("grades", "statistics_skill_level_math");
$pie_grade_tmpl = $templates->split_template("pie_grades", "statistics_skill_level_math");
$parent_skills_tmpl = $templates->split_template("parent_skills", "statistics_skill_level_math");
$skills_tmpl = $templates->split_template("skills", "statistics_skill_level_math");
...
foreach ($grades as $grade_number => $grade_info){
	...
	$user_skill_progress_info = select_items("math_user_skill_info", "skill_id", "smart_point", $common_where);
	...
	foreach ($skills_parents as $parent_id => $skill_parent_info){
		...
		foreach((array)$skills as $skill_id => $skill_info){
			...
			$skill_fields['progress_color'] = progress_color($skill_fields['progress']);
		...
		$skill_title_fields['progress_color'] = progress_color($skill_title_fields['progress']);
...
$out .= $templates->gen_module_html($replace_fields, "statistics_skill_level_math");
