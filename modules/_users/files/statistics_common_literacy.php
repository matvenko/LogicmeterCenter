...
global $out;
//*** search fields
$replace_fields['statistic_page'] = "statistics_common_literacy";
...
$replace_fields['_TEXTS_AMOUNT'] = _TEXTS_AMOUNT;
$replace_fields['_TOTAL_POINT'] = _TOTAL_POINT;
$replace_fields['_SKILL'] = _SKILL;
$replace_fields['_TEXT'] = _TEXT;
...
//***** list by date ***
$list_tmpl = $templates->split_template("list_by_date", "statistics_common_literacy");
...
$result = $query->select_sql("math_user_skill_info_by_day", "COUNT(DISTINCT text_id) AS texts_amount,
															`date`,
															`week`,
															`month`,
															SUM(tests_amount) AS tests_amount,
															SUM(true_answers) AS true_answers,
															SUM(spent_time) AS spent_time", $common_where." AND text_id != 0 AND `date` >= '{{date_from}}' AND `date` <= '{{date_to}}'", 
...
while($row = mysql_fetch_assoc($result)){
	...
	$skill_sum_info = $query->select_ar_sql("math_user_skill_info", "smart_point", $common_where." AND text_id = ".(int)$row['text_id']);
	...
	if($result_type == "week"){
		...
		$chart_data_texts_ar[(int)$row['date']."-".$row['week']] = $row['texts_amount'];
	}
 	elseif($result_type == "month"){
		...
		$chart_data_texts_ar[(int)$row['date']."-".$row['month']] = $row['texts_amount'];
	}
	else{
		$chart_data_ar[$row['date']] = round($row['spent_time'] / 60);
		$chart_data_texts_ar[$row['date']] = $row['texts_amount'];
	}
...
//***** list by skill ***
$list_tmpl = $templates->split_template("list_by_skill", "statistics_common_literacy");

$query->where_vars['date_from'] = $date_from;
$query->where_vars['date_to'] = $date_to;
$result = $query->select_sql("math_user_skill_info_by_day", "grade_id,
															skill_id,
															text_id,
															SUM(tests_amount) AS tests_amount,
															SUM(true_answers) AS true_answers,
															SUM(spent_time) AS spent_time", $common_where." AND text_id != 0 AND `date` >= '{{date_from}}' AND `date` <= '{{date_to}}'",
		"`date` DESC", "", "user_id, child_id, text_id");
...
while($row = mysql_fetch_assoc($result)){
	...
	$list_fields['smart_point'] = $skill_sum_info['smart_point'];
	$list_fields['progress_color'] = progress_color($skill_sum_info['smart_point']);
	$list_fields['text'] = $literacy->text_title($row['text_id']);
	$list_fields['wrong_answers'] = $row['tests_amount'] - $row['true_answers'];
	$list_fields['_HR'] = _HR;
	$list_fields['_MNT'] = _MNT;
	$list_fields['_SC'] = _SC;
...
$replace_fields['_CHART_TITLE_TEXTS'] = _CHART_TITLE_TEXTS;
$replace_fields['_LEFT_LABEL_TEXTS'] = _LEFT_LABEL_TEXTS;
...
while($last_day < date_to_unix($date_to)){	
	...
	$chart_texts_data[] = array_key_exists($chart_data_key, (array)$chart_data_texts_ar) ? $chart_data_texts_ar[$chart_data_key] : 0;

	$chart_unix_time = strtotime("+1 ".$result_type, $chart_unix_time);
}

$replace_fields['chart_data'] = implode(",", $chart_data);
$replace_fields['chart_texts_data'] = implode(",", $chart_texts_data);
$replace_fields['labels'] = ltrim($labels, ',');

$out .= $templates->gen_module_html($replace_fields, "statistics_common_literacy");
