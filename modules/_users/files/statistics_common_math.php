...
global $out;
//*** search fields
$replace_fields['statistic_page'] = "statistics_common_math";
...
//***** list by date ***
$list_tmpl = $templates->split_template("list_by_date", "statistics_common_math");
...
$result = $query->select_sql("math_user_skill_info_by_day", "COUNT(DISTINCT skill_id) AS skills_amount,
															`date`,
															`week`,
															`month`,
...
while($row = mysql_fetch_assoc($result)){
	...
	if($result_type == "week"){
		...
		$chart_data_skills_ar[(int)$row['date']."-".$row['week']] = $row['skills_amount'];
	}
 	elseif($result_type == "month"){
		...
		$chart_data_skills_ar[(int)$row['date']."-".$row['month']] = $row['skills_amount'];
	}
	else{
		$chart_data_ar[$row['date']] = round($row['spent_time'] / 60);
		$chart_data_skills_ar[$row['date']] = $row['skills_amount'];
	}
...
//***** list by skill ***
$list_tmpl = $templates->split_template("list_by_skill", "statistics_common_math");
...
$result = $query->select_sql("math_user_skill_info_by_day", "grade_id,
															...
															SUM(true_answers) AS true_answers,
															SUM(spent_time) AS spent_time", $common_where." AND text_id = 0 AND `date` >= '{{date_from}}' AND `date` <= '{{date_to}}'",
										"grade_id ASC, skill_id ASC", "", "user_id, child_id, skill_id");
...
$replace_fields['_CHART_TITLE_SKILLS'] = _CHART_TITLE_COMMON_SKILLS;
$replace_fields['_LEFT_LABEL'] = _LEFT_LABEL_COMMON;
$replace_fields['_LEFT_LABEL_SKILLS'] = _LEFT_LABEL_COMMON_SKILLS;
...
while($last_day < date_to_unix($date_to)){	
	...
	$chart_skills_data[] = array_key_exists($chart_data_key, (array)$chart_data_skills_ar) ? $chart_data_skills_ar[$chart_data_key] : 0;

	$chart_unix_time = strtotime("+1 ".$result_type, $chart_unix_time);
}

$replace_fields['chart_data'] = implode(",", $chart_data);
$replace_fields['chart_skills_data'] = implode(",", $chart_skills_data);
$replace_fields['labels'] = ltrim($labels, ',');

$out .= $templates->gen_module_html($replace_fields, "statistics_common_math");
