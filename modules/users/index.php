...
if(is_post('register')){
    ...
    $query_l->where_vars['subject'] = post('subject');
    $subject_info = $query_l->select_ar_sql("center_subjects", "*", "alias = '{{subject}}'");
    $subject = (int)$subject_info['id'] !== 0 ? post('subject') : "math";

    if(post('subject') == false){
		echo request_callback("error", _CHOOSE_DIRECTION, "form_field_error", array("field_id" => "choose_direction", "good_fields" => array()));
		exit;
	}
    ...
    //*** check tel
    $tel = preg_replace("/^995/", "", only_numbers(post('parent_mobile')));
    if(strlen($tel) !== 9){
        echo request_callback("error", _TEL_ERROR, "form_field_error", array("field_id" => "parent_mobile", "good_fields" => (array)$good_fields));
        exit;
    }
	...
	$query_l->where_vars['subject'] = substr(post('subject'), 0, 1);
	$children_amount = $query_l->amount_fields("center_class_children", "disabled = 0 AND subject = '{{subject}}' AND year = '".$math->current_lesson_year()."' AND grade = ".post_int('grade')." AND dayes_id = ".post_int('dayes')." AND hours_id = ".post_int('hours'));
	$query->change_db($sql_db);
    ...
    if (post('grade') === false ||
        ...
        echo request_callback("error", _CHOOSE_PACKAGE, "form_field_error", array("field_id" => "",
            "good_fields" => (array)$good_fields
        ));
        exit;
    }
    //**** check class places
    if ($children_amount >= $subject_info['max_place']) {
        echo request_callback("error", _CLASS_IS_FULL);
        exit;
    }

	//** saatebis gadakveta
	if(post_int('direction') == 1 && post_int('math_dayes') == post_int('literacy_dayes') && post_int('math_hours') == post_int('literacy_hours')){
    ...
    $fields_class['grade'] = post_int('grade');
    $fields_class['dayes_id'] = post_int('dayes');
    $fields_class['hours_id'] = post_int('hours');
	...
	$query->change_db($sql_db_l);
    $math->generate_package_center($child_id, (float)post('_payment_period'), $subject);
    $query->change_db($sql_db);
