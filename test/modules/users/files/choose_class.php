...
$subjects_ar = array('math' => _MATH_LITERACY, 'rlego' => _LEGO, 'individual' => _INDIVIDUAL);
...
if(get('action') == "calculate_price"){
	...
	$subject = 0;
	foreach($subjects_ar as $key => $value){
		if(get_int($key) == 1){
			$subject = $key;
			break;
		}
	}
	$dayes_id = get_int($subject.'_dayes_id');
	...
	if(max($children_amount) >= $class_max_children){
		...
		$month_payment = isset($math->config['center_1_monthly_paiment_'.$subject]) ? $math->config['center_1_monthly_paiment_'.$subject] : $math->config['center_1_monthly_paiment'];
		$payment_bh_lessons = get_int('period') * 8 > $remaining_lessons ? $remaining_lessons * $month_payment / 8 : $month_payment * get_int('period');
...
$replace_fields['direction_4'] = input_form("direction", "single_radio", $edit_value, 4);
...
foreach($subjects_ar as $subject => $subject_title){
	...
	$result_dayes = $query_l->select_sql("center_lesson_dayes", "*", $subject." = 1 AND math = 1", "sort ASC");
