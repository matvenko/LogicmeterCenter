...
if(is_post('register')){
	...
	//***** check packages ***
	if(post('grade') === false || post('schedule') == false || post('payment_period') == false){
	...
	$schedule = explode('-', post('schedule'));
	$fields_class['dayes_id'] = $schedule['0'];
	$fields_class['hours_id'] = $schedule[1];
	$fields_class['subject'] = 'm';
	...
	$query->sql_db = $sql_db_l;
	$math->generate_package_center($child_id, post_int('payment_period'));
	$query->sql_db = $sql_db;
    ...
    echo request_callback("ok", $global_conf['location_logicmeter']."index.php?module=".$module."&page=profile&type=make_payment&session_id=".session_id());
	exit;
}
//************************************
